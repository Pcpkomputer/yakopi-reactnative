...
export default function ListPlotBoundaringScreen(props){
    ...
    let fetchList = async () =>{
        setListLoading(true);
        let request = await fetch(`${endpoint}/plot-boundering`,{
            method:"GET",
            headers:{
    ...
    return (
        <View style={{flex:1}}>
            ...
            activeOpacity={0.6}
            onPress={()=>{
                props.navigation.navigate("InputPlotBoundaring");
            }}
                ...
                <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                            ...
                            activeOpacity={0.7}
                            onPress={()=>{
                                props.navigation.navigate("DetailPlotBoundaring",{item:item});
                            }}
                            key={item.id_plot_boundaring}>
                            <LinearGradient
                            ...
                            style={{flexDirection:"row",marginHorizontal:EStyleSheet.value("20rem"),borderRadius:EStyleSheet.value("5rem")}}>
                                <View style={{paddingHorizontal:EStyleSheet.value("20rem"),justifyContent:"center",alignItems:"center",paddingVertical:EStyleSheet.value("20rem")}}>
                                    <Text style={{color:"white",fontSize:EStyleSheet.value("20rem"),fontWeight:"bold"}}>{item.id_plot_boundaring}</Text>
                                </View>
                            ...
                            <View style={{marginHorizontal:EStyleSheet.value("20rem"),flexDirection:"row",justifyContent:"space-around",padding:EStyleSheet.value("10rem"),backgroundColor:"#DDDDDD"}}>
                                <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindPlotBoundaring",{id_plot_boundaring:item.id_plot_boundaring,status:item.status});
                                }}
                                ...
                                <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("AssetsKK6",{type:"video",id_agroforest_kt6:item.id_agroforest_kt6,status:item.status});
                                }}
                                ...
                                <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("AssetsKK6",{type:"drone",id_agroforest_kt6:item.id_agroforest_kt6,status:item.status});
                                }}
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_plot_boundaring;
                                                    let request = await fetch(`${endpoint}/update-plot-boundering-verifikasi_1`,{
                                                        method:"POST",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_plot_boundaring;
                                                    let request = await fetch(`${endpoint}/update-plot-boundering-verifikasi_2`,{
                                                        method:"POST",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_plot_boundaring;
                                                    let request = await fetch(`${endpoint}/approve-plot-boundering`,{
                                                        method:"POST",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_plot_boundaring;
                                                    let request = await fetch(`${endpoint}/delete-plot-boundering`,{
                                                        method:"DELETE",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
