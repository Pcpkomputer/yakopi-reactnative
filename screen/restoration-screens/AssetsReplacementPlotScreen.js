...
function AssetsVideo(props){
    ...
    const fetchVideo = async()=>{
        ...
        let request = await fetch(`${endpoint}/video-replacement-plot`,{
            method:"POST",
            headers:{
    ...
    useEffect(()=>{
        props.navigation.setOptions({
            headerTitle: `Video - ${props.route.params.id_detail_replacement_plot}`,
        });
    },[])
                    ...
                    <View style={{...shadow,backgroundColor:"white",width:Dimensions.get("screen").width-EStyleSheet.value("60rem"),borderRadius:EStyleSheet.value("5rem"),padding:EStyleSheet.value("20rem")}}>
                        ...
                        onPress={async ()=>{
                            ...
                            if(gambar.type==="success"){
                                ...
                                form.append("file_replacement_plot_video",photo);
                                let request = await fetch(`https://sispro-yakopi.org/endpoint/dokumentasiVideoReplacementPlot`,{
                                    method:"POST",
                                    body:form
                                });
                                let response = await request.json();
            
                                let url = `/assets/img/videoReplacementPlot/${response.result.orig_name}`;

                                let id = props.route.params.id_detail_replacement_plot;

                                
                                let req2 = await fetch(`${endpoint}/add-video-replacement-plot`,{
                                    method:"POST",
                                    headers:{
                                        "authorization":`Bearer ${globalContext.credentials.token}`,
                                        "content-type":"application/json"
                                    ...
                                    body:JSON.stringify({
                                        id_detail_replacement_plot:id,
                                        keterangan_replacement_plot_video:keterangan,
                                        link_replacement_plot_video:"",
                                        file_replacement_plot_video:url
                                    })
                                });
                        ...
                        return (
                            <View style={{marginHorizontal:EStyleSheet.value("20rem"),overflow:"hidden",borderRadius:EStyleSheet.value("3rem"),marginBottom:EStyleSheet.value("15rem")}}>
                                <View style={{backgroundColor:"#e8e8e8",height:EStyleSheet.value("200rem")}}>
                                    {
                                        (item.link_replacement_plot_video===null || item.link_replacement_plot_video==="") ?
                                        <Video
                                        style={{flex:1}}
                                        source={{
                                        uri: `${endpoint.replace(/api(.?)/g,"")}${item.file_replacement_plot_video}`,
                                        }}
                                        useNativeControls
                                        resizeMode="contain"/>
                                        :
                                        ...
                                        style={{flex:1}}
                                        source={{
                                        uri: item.link_replacement_plot_video,
                                        }}
                                        useNativeControls
                                        resizeMode="contain"/>
                                    }
                                ...
                                <View style={{zIndex:10,paddingHorizontal:EStyleSheet.value("20rem"),paddingVertical:EStyleSheet.value("15rem")}}>
                                    <Text style={{color:"white",fontSize:EStyleSheet.value("20rem")}}>{item.keterangan_replacement_plot_video}</Text>
                                </View>
                                <LinearGradient
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_replacement_plot_video;
                                                    let request = await fetch(`${endpoint}/delete-video-replacement-plot`,{
                                                        method:"DELETE",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
...
function AssetsDrone(props){
    ...
    const fetchImage = async()=>{
        setImageLoading(true);
        let id = props.route.params.id_detail_replacement_plot;
        let request = await fetch(`${endpoint}/drone-replacement-plot`,{
            method:"POST",
            headers:{
                "authorization":`Bearer ${globalContext.credentials.token}`,
                "content-type":"application/json"
    ...
    useEffect(()=>{
        props.navigation.setOptions({
            headerTitle: `Drone - ${props.route.params.id_detail_replacement_plot}`,
        });
    },[])
...

{
                    ...
                    <View style={{...shadow,backgroundColor:"white",width:Dimensions.get("screen").width-EStyleSheet.value("60rem"),borderRadius:EStyleSheet.value("5rem"),padding:EStyleSheet.value("20rem")}}>
                        ...
                        onPress={async ()=>{
                            ...
                            if(!gambar.canceled){
                                ...
                                form.append("file_replacement_plot_drone",photo);
                                let request = await fetch(`https://sispro-yakopi.org/endpoint/dokumentasiDroneReplacementPlot`,{
                                    method:"POST",
                                    body:form
                                });
                                let response = await request.json();
            
                                let url = `/assets/img/droneReplacementPlot/${response.result.orig_name}`;

                                let id = props.route.params.id_detail_replacement_plot;

                                
                                let req2 = await fetch(`${endpoint}/add-drone-replacement-plot`,{
                                    method:"POST",
                                    headers:{
                                        "authorization":`Bearer ${globalContext.credentials.token}`,
                                        "content-type":"application/json"
                                    ...
                                    body:JSON.stringify({
                                        id_detail_replacement_plot:id,
                                        keterangan_replacement_plot_drone:keterangan,
                                        link_replacement_plot_drone:"",
                                        file_replacement_plot_drone:url
                                    })
                                });
                                let res2 = await req2.json();
...

            {
                ...
                <FlatList
                    ...
                    data={image}
                    renderItem={({item,index})=>{
                        console.log(`${endpoint.replace(/api(.?)/g,"")}${item.file_replacement_plot_drone}`);
                        return (
                            <View style={{marginHorizontal:EStyleSheet.value("20rem"),overflow:"hidden",borderRadius:EStyleSheet.value("3rem"),marginBottom:EStyleSheet.value("15rem")}}>
                                <View style={{backgroundColor:"#e8e8e8",height:EStyleSheet.value("200rem")}}>
                                    {
                                        (item.link_replacement_plot_drone===null || item.link_replacement_plot_drone==="") ?
                                        <Image style={{width:"100%",height:"100%"}} source={{uri:`${endpoint.replace(/api(.?)/g,"")}${item.file_replacement_plot_drone}`}}></Image>
                                        :
                                        <Image style={{width:"100%",height:"100%"}} source={{uri:item.link_replacement_plot_drone}}></Image>
                                    }
                                </View>
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_replacement_plot_drone;
                                                    let request = await fetch(`${endpoint}/delete-drone-replacement-plot`,{
                                                        method:"DELETE",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
...
function AssetsImage(props){
    ...
    const fetchImage = async()=>{
        setImageLoading(true);
        let id = props.route.params.id_detail_replacement_plot;
        let request = await fetch(`${endpoint}/photo-replacement-plot`,{
            method:"POST",
            headers:{
                "authorization":`Bearer ${globalContext.credentials.token}`,
                "content-type":"application/json"
    ...
    useEffect(()=>{
        props.navigation.setOptions({
            headerTitle: `Photo - ${props.route.params.id_detail_replacement_plot}`,
        });
    },[])
                    ...
                    renderItem={({item,index})=>{
                        return (
                            <View style={{marginHorizontal:EStyleSheet.value("20rem"),overflow:"hidden",borderRadius:EStyleSheet.value("3rem"),marginBottom:EStyleSheet.value("15rem")}}>
                                <View style={{backgroundColor:"#e8e8e8",height:EStyleSheet.value("200rem")}}>
                                    {
                                        (item.link_replacement_plot_photo===null || item.link_replacement_plot_photo==="") ?
                                        <Image style={{width:"100%",height:"100%"}} source={{uri:`${endpoint.replace(/api(.?)/g,"")}${item.file_replacement_plot_photo}`}}></Image>
                                        :
                                        <Image style={{width:"100%",height:"100%"}} source={{uri:item.link_replacement_plot_photo}}></Image>
                                    }
                                </View>
                                <View style={{position:"absolute",zIndex:10,bottom:EStyleSheet.value("20rem"),paddingHorizontal:EStyleSheet.value("20rem")}}>
                                    <Text style={{color:"white",fontSize:EStyleSheet.value("20rem")}}>{item.keterangan_replacement_plot_photo}</Text>
                                </View>
                                <LinearGradient
                                style={{position:"absolute",bottom:0,width:"100%",height:"50%"}}
                                colors={['transparent', 'rgba(0,0,0,0.5)']}>
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_replacement_plot_photo;
                                                    let request = await fetch(`${endpoint}/delete-photo-replacement-plot`,{
                                                        method:"DELETE",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
...
export default function AssetsReplacementPlotScreen(props){
