...
export default function InputDetailGrowthOfflineScreen(props){
    ...
    const id_growth = props.route.params.id_growth;
            ...
            >
              ...
              <TouchableOpacity 
                ...
                onPress={async () => {
                    ...
                    payload.id = props.route.params.id_growth;
                    let payloadArray = [payload];

                    try {
                    const storedKT7Kind = await AsyncStorage.getItem("KT7Kind");
                    const parsedStoredKT7Kind = JSON.parse(storedKT7Kind) || [];

                    if (parsedStoredKT7Kind.length > 0) {
                        const updatedKT7Kind = [...parsedStoredKT7Kind, ...payloadArray];
                        await AsyncStorage.setItem("KT7Kind", JSON.stringify(updatedKT7Kind));
                        setKT7Kind(updatedKT7Kind);
                    } else {
                        await AsyncStorage.setItem("KT7Kind", JSON.stringify(payloadArray));
                        setKT7Kind(payloadArray);
                    }

                    setSmokeScreenOpened(false);
                    alert('Berhasil menyimpan data ke local');
                    props.navigation.navigate("KindGrowthOffline", { id_growth: props.route.params.id_growth });
                    } catch (error) {
