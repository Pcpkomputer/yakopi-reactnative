...
export default function KindTransportOfflineScreen(props){
    ...
    let fetchList = async () =>{
        setListLoading(true);
        let list = await AsyncStorage.getItem("KT5Kind");
        list = JSON.parse(list);
        if(list===null){
            list = [];
        }
        list = list.filter((item)=>{
            return item.id === id_transport;
        })
        setList(list);
    ...
    return (
      <View style={{flex:1, backgroundColor:'#fff'}}>
        ...
        <TouchableOpacity 
            activeOpacity={0.6}
            onPress={()=>{
                props.navigation.navigate("InputDetailTransportOffline",{ id_transport:props.route.params.id_transport });
            }}
                      ...
                      [
                          ...
                          { 
                              ...
                              onPress: async () => {
                                  ...
                                  list.splice(index, 1);
                                  await AsyncStorage.setItem("KT5Kind", JSON.stringify(list));
