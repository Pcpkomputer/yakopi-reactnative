...
export default function InputDetailReplacementPlotOfflineScreen(props){
    ...
    const [schema, setSchema] = useState([
        {
            form:"id_replacement_plot",
            value:props.route.params.id_replacement_plot,
        },
        {
            type:"textinput",
            label:"Kode Site (100%)",
            ...
            >
               ...
               <TouchableOpacity 
                ...
                onPress={async () => {
                    ...
                    payload.id = props.route.params.id_replacement_plot;
                    let payloadArray = [payload];

                    try {
                    const storedKT10Kind = await AsyncStorage.getItem("KT10Kind");
                    const parsedStoredKT10Kind = JSON.parse(storedKT10Kind) || [];

                    if (parsedStoredKT10Kind.length > 0) {
                        const updatedKT10Kind = [...parsedStoredKT10Kind, ...payloadArray];
                        await AsyncStorage.setItem("KT10Kind", JSON.stringify(updatedKT10Kind));
                        setKT10Kind(updatedKT10Kind);
                    } else {
                        await AsyncStorage.setItem("KT10Kind", JSON.stringify(payloadArray));
                        setKT10Kind(payloadArray);
                    }

                    setSmokeScreenOpened(false);
                    alert('Berhasil menyimpan data ke local');
                    props.navigation.navigate("KindReplacementPlotOffline",{id_replacement_plot:props.route.params.id_replacement_plot});
                    } catch (error) {
