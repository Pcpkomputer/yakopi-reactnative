...
export default function KindReplantingOffline(props){
  ...
  let fetchList = async () =>{
      setListLoading(true);
      let list = await AsyncStorage.getItem("KT8Kind");
      list = JSON.parse(list);
      if(list===null){
          list = [];
      }
      list = list.filter((item)=>{
          return item.id === id_replanting;
      })
      setList(list);
    ...
    return (
      <View style={{flex:1, backgroundColor:'#fff'}}>
        <TouchableOpacity 
            activeOpacity={0.6}
            onPress={()=>{
                props.navigation.navigate("InputDetailReplantingOffline",{ id_replanting:props.route.params.id_replanting });
            }}
                      ...
                      [
                          ...
                          { 
                              ...
                              onPress: async () => {
                                  ...
                                  list.splice(index, 1);
                                  await AsyncStorage.setItem("KT8Kind", JSON.stringify(list));
