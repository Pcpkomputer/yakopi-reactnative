...
export default function ListReplacementPlotOfflineScreen(props){
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("KT10");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputReplacementPlotOffline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
                     ...
                     list.map((item,index)=>{
                         return (
                            ...
                            <View style={{marginHorizontal:EStyleSheet.value("20rem"),flexDirection:"row",justifyContent:"space-around",padding:EStyleSheet.value("10rem"),backgroundColor:"#DDDDDD"}}>
                                        <TouchableOpacity 
                                            onPress={()=>{
                                                props.navigation.navigate("KindReplacementPlotOffline",{id_replacement_plot:item.id});
                                            }}
                                        ...
                                        onPress={async () => {
                                            ...
                                            [
                                                ...
                                                onPress: async () => {
                                                    ...
                                                    if (list.length > 0) {
                                                    list.splice(index, 1);
                                                    await AsyncStorage.setItem("KT10", JSON.stringify(list));
                                                    setList(list);
                                                    setListLoading(false);
                                            ...
                                            onPress={async () => {
                                                ...
                                                try {
                                                    ...
                                                    if (response.success) {
                                                        ...
                                                        // Update KT10Kind
                                                        let id = response.id_replacement_plot;
                                                        let id_replacement_plot = data.id;

                                                        let KT10Kind = await AsyncStorage.getItem("KT10Kind");
                                                        KT10Kind = JSON.parse(KT10Kind) || [];

                                                        let KT10KindFiltered = KT10Kind.filter(item => item.id_replacement_plot === id_replacement_plot);

                                                        if (KT10KindFiltered.length > 0) {
                                                            KT10KindFiltered.forEach(item => {
                                                                item.id_replacement_plot = id;
                                                            });
                                                        } else {
                                                            // KT10Kind tidak ada, tidak perlu mengupload
                                                        ...
                                                        let KT10KindFiltered2 = KT10Kind.filter(item => item.id_replacement_plot === id);


                                                        // Jika lebih dari 1, lakukan loop
                                                        if (KT10KindFiltered2.length > 0) {
                                                            for (let i = 0; i < KT10KindFiltered2.length; i++) {
                                                                let request1 = await fetch(`${endpoint}/add-kind-replacement-plot`,{
                                                                    method: "POST",
                                                                    headers: {
                                                                        "authorization": `Bearer ${globalContext.credentials.token}`,
                                                                        "content-type": "application/json"
                                                                    },
                                                                    body: JSON.stringify(KT10KindFiltered2[i])
                                                                });

                                                                let response1 = await request1.json();

                                                                if (response1.success) {
                                                                    // Hapus data KT10Kind yang terkait dengan KT10 yang dihapus
                                                                    let KT10KindFiltered = KT10Kind.filter(item => item.id_replacement_plot !== id);
                                                                    await AsyncStorage.setItem("KT10Kind", JSON.stringify(KT10KindFiltered));

                                                                    setList(list);
                                                                    setListLoading(false);
                                                                    alert("Data berhasil diupload");
                                                                    fetchList(); // Add a function to reload the list
                                                                    let updatedKT10Kind = [...KT10Kind, KT10KindFiltered]; // Gantilah 'data' dengan properti yang sesuai
                                                                    setKT10Kind(updatedKT10Kind);
                                                                } else {
                                                            ...
                                                            KT10KindFiltered2 = KT10KindFiltered2[0];
                                                            ...
                                                            try {
                                                                let request1 = await fetch(`${endpoint}/add-kind-replacement-plot`,{
                                                                    ...
                                                                    body: JSON.stringify(KT10KindFiltered2)
                                                                });
