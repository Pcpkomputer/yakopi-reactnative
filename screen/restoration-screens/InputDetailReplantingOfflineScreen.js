...
export default function InputDetailReplantingOfflineScreen(props){
    ...
    const id_replanting = props.route.params.id_replanting;
    ...
    const [schema, setSchema] = useState([
        {
            form:"id_replanting",
            value:props.route.params.id_replanting,
        },
        {
            type:"textinput",
            label:"Kode Site",
            ...
            >
               ...
               <TouchableOpacity 
                ...
                onPress={async () => {
                    ...
                    payload.id = props.route.params.id_replanting;
                    let payloadArray = [payload];

                    try {
                    const storedKT8Kind = await AsyncStorage.getItem("KT8Kind");
                    const parsedStoredKT8Kind = JSON.parse(storedKT8Kind) || [];

                    if (parsedStoredKT8Kind.length > 0) {
                        const updatedKT8Kind = [...parsedStoredKT8Kind, ...payloadArray];
                        await AsyncStorage.setItem("KT8Kind", JSON.stringify(updatedKT8Kind));
                        setKT8Kind(updatedKT8Kind);
                    } else {
                        await AsyncStorage.setItem("KT8Kind", JSON.stringify(payloadArray));
                        setKT8Kind(payloadArray);
                    }

                    setSmokeScreenOpened(false);
                    alert('Berhasil menyimpan data ke local');
                    props.navigation.navigate("KindReplantingOffline",{id_replanting:props.route.params.id_replanting});
                    } catch (error) {
