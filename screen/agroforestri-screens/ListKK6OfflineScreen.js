...
export default function ListKK6OfflineScreen(props){

    const focused = useIsFocused();

    const globalContext = useContext(GlobalContext);
    const [credentials,setCredentials] = useState(globalContext.credentials);
    const [KK6, setKK6] = useState(globalContext.KK6);
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("KK6");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputKK6Offline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
          ...
          <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                               ...
                               <View style={{paddingHorizontal:EStyleSheet.value("20rem"),justifyContent:"center",alignItems:"center",paddingVertical:EStyleSheet.value("20rem")}}>
                                ...
                                <View style={{flexDirection:"column",flex:1}}>
                                    ...
                                    <View style={{padding:EStyleSheet.value("10rem")}}>
                                        <Text style={{color:"white",fontWeight:"bold",fontSize:EStyleSheet.value("16rem"),paddingBottom:EStyleSheet.value("10rem")}}>MONITORING HASIL TANAM (PLOT BOUNDARING)</Text>
                                    </View>
                                </View>
                                <TouchableOpacity 
                                activeOpacity={0.8}
                            ...
                            <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindKK6Offline",{id_agroforest_kt6:item.id,status:item.status});
                                }}
                                    ...
                                    onPress={async () => {
                                        ...
                                        [
                                            ...
                                            onPress: async () => {
                                                ...
                                                if (list.length > 0) {
                                                list.splice(index, 1);
                                                await AsyncStorage.setItem("KK6", JSON.stringify(list));
                                                setList(list);
                                                setListLoading(false);
                                    ...
                                    isConnected ? (
                                        ...
                                        onPress={async () => {
                                            ...
                                            try {
                                                ...
                                                let request = await fetch(`${endpoint}/agroforest-kt6`, {
                                                    method: "POST",
                                                    headers: {
                                                ...
                                                if (response.success) {
                                                    ...
                                                    // Update KK6Kind
                                                    let id = response.id_agroforest_kt6;
                                                    let id_agroforest_kt6 = data.id;

                                                    let KK6Kind = await AsyncStorage.getItem("KK6Kind");
                                                    KK6Kind = JSON.parse(KK6Kind) || [];

                                                    let KK6KindFiltered = KK6Kind.filter(item => item.id_agroforest_kt6 === id_agroforest_kt6);

                                                    if (KK6KindFiltered.length > 0) {
                                                        KK6KindFiltered.forEach(item => {
                                                            item.id_agroforest_kt6 = id;
                                                        });
                                                        await AsyncStorage.setItem("KK6Kind", JSON.stringify(KK6Kind));
                                                    } else {
                                                        // KK6Kind tidak ada, tidak perlu mengupload
                                                    ...
                                                    let KK6KindFiltered2 = KK6Kind.filter(item => item.id_agroforest_kt6 === id);

                                                    if (KK6KindFiltered2.length > 0) {
                                                        for (let i = 0; i < KK6KindFiltered2.length; i++) {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt6`, {
                                                                method: "POST",
                                                                headers: {
                                                                    "authorization": `Bearer ${globalContext.credentials.token}`,
                                                                    "content-type": "application/json"
                                                                },
                                                                body: JSON.stringify(KK6KindFiltered2[i])
                                                            });
                                                            ...
                                                            if (response1.success) {
                                                                ...
                                                                let KK6Kind = await AsyncStorage.getItem("KK6Kind");
                                                                KK6Kind = JSON.parse(KK6Kind) || [];
                                                                let KK6KindFiltered = KK6Kind.filter(item => item.id_agroforest_kt6 !== id);
                                                                await AsyncStorage.setItem("KK6Kind", JSON.stringify(KK6KindFiltered));
                                                        ...
                                                        KK6KindFiltered2 = KK6KindFiltered2[0];
                                                        ...
                                                        try {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt6`, {
                                                                ...
                                                                body: JSON.stringify(KK6KindFiltered2)
                                                            });
