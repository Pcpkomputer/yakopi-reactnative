...
export default function ListKK3OfflineScreen(props){

    const focused = useIsFocused();

    const globalContext = useContext(GlobalContext);
    const [credentials,setCredentials] = useState(globalContext.credentials);
    const [KK3, setKK3] = useState(globalContext.KK3);
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("KK3");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputKK3Offline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
          ...
          <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                            ...
                            <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindKK3Offline",{id_agroforest_kt3:item.id,status:item.status});
                                }}
                                    ...
                                    onPress={async () => {
                                        ...
                                        [
                                            ...
                                            onPress: async () => {
                                                ...
                                                if (list.length > 0) {
                                                list.splice(index, 1);
                                                await AsyncStorage.setItem("KK3", JSON.stringify(list));
                                                setList(list);
                                                setListLoading(false);
                                    ...
                                    isConnected ? (
                                        ...
                                        onPress={async () => {
                                            ...
                                            try {
                                                ...
                                                let request = await fetch(`${endpoint}/agroforest-kt3`, {
                                                    method: "POST",
                                                    headers: {
                                                ...
                                                if (response.success) {
                                                    ...
                                                    // Update KK3Kind
                                                    let id = response.id_agroforest_kt3;
                                                    let id_agroforest_kt3 = data.id;

                                                    let KK3Kind = await AsyncStorage.getItem("KK3Kind");
                                                    KK3Kind = JSON.parse(KK3Kind) || [];

                                                    let KK3KindFiltered = KK3Kind.filter(item => item.id_agroforest_kt3 === id_agroforest_kt3);

                                                    if (KK3KindFiltered.length > 0) {
                                                        KK3KindFiltered.forEach(item => {
                                                            item.id_agroforest_kt3 = id;
                                                        });
                                                        await AsyncStorage.setItem("KK3Kind", JSON.stringify(KK3Kind));
                                                    } else {
                                                        // KK3Kind tidak ada, tidak perlu mengupload
                                                    ...
                                                    let KK3KindFiltered2 = KK3Kind.filter(item => item.id_agroforest_kt3 === id);

                                                    if (KK3KindFiltered2.length > 0) {
                                                        for (let i = 0; i < KK3KindFiltered2.length; i++) {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt3`, {
                                                                method: "POST",
                                                                headers: {
                                                                    "authorization": `Bearer ${globalContext.credentials.token}`,
                                                                    "content-type": "application/json"
                                                                },
                                                                body: JSON.stringify(KK3KindFiltered2[i])
                                                            });
                                                            ...
                                                            if (response1.success) {
                                                                ...
                                                                let KK3Kind = await AsyncStorage.getItem("KK3Kind");
                                                                KK3Kind = JSON.parse(KK3Kind) || [];
                                                                let KK3KindFiltered = KK3Kind.filter(item => item.id_agroforest_kt3 !== id);
                                                                await AsyncStorage.setItem("KK3Kind", JSON.stringify(KK3KindFiltered));
                                                        ...
                                                        KK3KindFiltered2 = KK3KindFiltered2[0];
                                                        ...
                                                        try {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt3`, {
                                                                ...
                                                                body: JSON.stringify(KK3KindFiltered2)
                                                            });
