...
export default function InputDetailKK4OfflineScreen(props){
    ...
    const [KK4Kind, setKK4Kind] = useState(globalContext.KK4Kind);
    const id_agroforest_kt4 = props.route.params.id_agroforest_kt4;
    ...
    const [schema, setSchema] = useState([
        {
            form:"id_agroforest_kt4",
            value:props.route.params.id_agroforest_kt4,
        },
        {
            type:"dateinput",
            label:"Tanggal",
            value:"",
            form:"date_kt4_detail",
            required:true
        },
        {
            type:"textNumber",
            ...
            >
               ...
               <TouchableOpacity 
                ...
                onPress={async () => {
                    ...
                    payload.id = props.route.params.id_agroforest_kt4;
                    let payloadArray = [payload];

                    try {
                    const storedKK4Kind = await AsyncStorage.getItem("KK4Kind");
                    const parsedStoredKK4Kind = JSON.parse(storedKK4Kind) || [];

                    if (parsedStoredKK4Kind.length > 0) {
                        const updatedKK4Kind = [...parsedStoredKK4Kind, ...payloadArray];
                        await AsyncStorage.setItem("KK4Kind", JSON.stringify(updatedKK4Kind));
                        setKK4Kind(updatedKK4Kind);
                    } else {
                        await AsyncStorage.setItem("KK4Kind", JSON.stringify(payloadArray));
                        setKK4Kind(payloadArray);
                    }

                    setSmokeScreenOpened(false);
                    alert('Berhasil menyimpan data ke local');
                    props.navigation.navigate("KindKK4Offline", { id_agroforest_kt4: props.route.params.id_agroforest_kt4 });
                    } catch (error) {
