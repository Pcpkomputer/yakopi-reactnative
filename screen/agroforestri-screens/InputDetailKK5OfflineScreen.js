...
export default function InputDetailKK5OfflineScreen(props){
    ...
    const id_agroforest_kt5 = props.route.params.id_agroforest_kt5;
    ...
    const [schema, setSchema] = useState([
        {
            form:"id_agroforest_kt5",
            value:props.route.params.id_agroforest_kt5,
        },
        {
            type:"dateinput",
            label:"Tanggal",
            value:"",
            form:"date_kt5_detail",
            required:true
        },
        {
            type:"textNumber",
            ...
            >
               ...
               <TouchableOpacity 
                ...
                onPress={async () => {
                    ...
                    payload.id = props.route.params.id_agroforest_kt5;
                    let payloadArray = [payload];

                    try {
                    const storedKK5Kind = await AsyncStorage.getItem("KK5Kind");
                    const parsedStoredKK5Kind = JSON.parse(storedKK5Kind) || [];

                    if (parsedStoredKK5Kind.length > 0) {
                        const updatedKK5Kind = [...parsedStoredKK5Kind, ...payloadArray];
                        await AsyncStorage.setItem("KK5Kind", JSON.stringify(updatedKK5Kind));
                        setKK5Kind(updatedKK5Kind);
                    } else {
                        await AsyncStorage.setItem("KK5Kind", JSON.stringify(payloadArray));
                        setKK5Kind(payloadArray);
                    }

                    setSmokeScreenOpened(false);
                    alert('Berhasil menyimpan data ke local');
                    props.navigation.navigate("KindKK5Offline", { id_agroforest_kt5: props.route.params.id_agroforest_kt5 });
                    } catch (error) {
