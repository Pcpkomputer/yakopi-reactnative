...
export default function ListKK1OfflineScreen(props){

    const focused = useIsFocused();

    const globalContext = useContext(GlobalContext);
    const [credentials,setCredentials] = useState(globalContext.credentials);
    const [KK1, setKK1] = useState(globalContext.KK1);
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("KK1");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputKK1Offline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
                                ...
                                onPress={async () => {
                                    ...
                                    [
                                        ...
                                        onPress: async () => {
                                            ...
                                            if (list.length > 0) {
                                                list.splice(index, 1);
                                                await AsyncStorage.setItem("KK1", JSON.stringify(list));
                                                setKK1(list);
                                                setList(list);
                                                setListLoading(false);
