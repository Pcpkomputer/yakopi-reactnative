...
export default function ListKK7OfflineScreen(props){

    const focused = useIsFocused();

    const globalContext = useContext(GlobalContext);
    const [credentials,setCredentials] = useState(globalContext.credentials);
    const [KK7, setKK7] = useState(globalContext.KK7);
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("KK7");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputKK7Offline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
          ...
          <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                            ...
                            <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindKK7Offline",{id_agroforest_kt7:item.id,status:item.status});
                                }}
                                    ...
                                    onPress={async () => {
                                        ...
                                        [
                                            ...
                                            onPress: async () => {
                                                ...
                                                if (list.length > 0) {
                                                list.splice(index, 1);
                                                await AsyncStorage.setItem("KK7", JSON.stringify(list));
                                                setList(list);
                                                setListLoading(false);
                                    ...
                                    isConnected ? (
                                        ...
                                        onPress={async () => {
                                            ...
                                            try {
                                                ...
                                                let request = await fetch(`${endpoint}/agroforest-kt7`, {
                                                    method: "POST",
                                                    headers: {
                                                ...
                                                if (response.success) {
                                                    ...
                                                    // Update KK7Kind
                                                    let id = response.id_agroforest_kt7;
                                                    let id_agroforest_kt7 = data.id;

                                                    let KK7Kind = await AsyncStorage.getItem("KK7Kind");
                                                    KK7Kind = JSON.parse(KK7Kind) || [];

                                                    let KK7KindFiltered = KK7Kind.filter(item => item.id_agroforest_kt7 === id_agroforest_kt7);

                                                    if (KK7KindFiltered.length > 0) {
                                                        KK7KindFiltered.forEach(item => {
                                                            item.id_agroforest_kt7 = id;
                                                        });
                                                        await AsyncStorage.setItem("KK7Kind", JSON.stringify(KK7Kind));
                                                    } else {
                                                        // KK7Kind tidak ada, tidak perlu mengupload
                                                    ...
                                                    let KK7KindFiltered2 = KK7Kind.filter(item => item.id_agroforest_kt7 === id);

                                                    if (KK7KindFiltered2.length > 0) {
                                                        for (let i = 0; i < KK7KindFiltered2.length; i++) {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt7`, {
                                                                method: "POST",
                                                                headers: {
                                                                    "authorization": `Bearer ${globalContext.credentials.token}`,
                                                                    "content-type": "application/json"
                                                                },
                                                                body: JSON.stringify(KK7KindFiltered2[i])
                                                            });
                                                            ...
                                                            if (response1.success) {
                                                                ...
                                                                let KK7Kind = await AsyncStorage.getItem("KK7Kind");
                                                                KK7Kind = JSON.parse(KK7Kind) || [];
                                                                let KK7KindFiltered = KK7Kind.filter(item => item.id_agroforest_kt7 !== id);
                                                                await AsyncStorage.setItem("KK7Kind", JSON.stringify(KK7KindFiltered));
                                                        ...
                                                        KK7KindFiltered2 = KK7KindFiltered2[0];
                                                        ...
                                                        try {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt7`, {
                                                                ...
                                                                body: JSON.stringify(KK7KindFiltered2)
                                                            });
