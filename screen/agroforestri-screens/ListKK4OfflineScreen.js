...
export default function ListKK4OfflineScreen(props){

    const focused = useIsFocused();

    const globalContext = useContext(GlobalContext);
    const [credentials,setCredentials] = useState(globalContext.credentials);
    const [KK4, setKK4] = useState(globalContext.KK4);
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("KK4");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputKK4Offline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
          ...
          <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                               ...
                               <View style={{paddingHorizontal:EStyleSheet.value("20rem"),justifyContent:"center",alignItems:"center",paddingVertical:EStyleSheet.value("20rem")}}>
                                ...
                                <View style={{flexDirection:"column",flex:1}}>
                                    ...
                                    <View style={{padding:EStyleSheet.value("10rem")}}>
                                        <Text style={{color:"white",fontWeight:"bold",fontSize:EStyleSheet.value("16rem"),paddingBottom:EStyleSheet.value("10rem")}}>KEGIATAN PENANAMAN TANAMAN AGROFORESTRI</Text>
                                    </View>
                                </View>
                                <TouchableOpacity 
                                activeOpacity={0.8}
                            ...
                            <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindKK4Offline",{id_agroforest_kt4:item.id,status:item.status});
                                }}
                                    ...
                                    onPress={async () => {
                                        ...
                                        [
                                            ...
                                            onPress: async () => {
                                                ...
                                                if (list.length > 0) {
                                                list.splice(index, 1);
                                                await AsyncStorage.setItem("KK4", JSON.stringify(list));
                                                setList(list);
                                                setListLoading(false);
                                        ...
                                        onPress={async () => {
                                            ...
                                            try {
                                                ...
                                                if (response.success) {
                                                    ...
                                                    // Update KK4Kind
                                                    let id = response.id_agroforest_kt4;
                                                    let id_agroforest_kt4 = data.id;

                                                    let KK4Kind = await AsyncStorage.getItem("KK4Kind");
                                                    KK4Kind = JSON.parse(KK4Kind) || [];

                                                    let KK4KindFiltered = KK4Kind.filter(item => item.id_agroforest_kt4 === id_agroforest_kt4);

                                                    if (KK4KindFiltered.length > 0) {
                                                        KK4KindFiltered.forEach(item => {
                                                            item.id_agroforest_kt4 = id;
                                                        });
                                                        await AsyncStorage.setItem("KK4Kind", JSON.stringify(KK4Kind));
                                                    } else {
                                                        // KK4Kind tidak ada, tidak perlu mengupload
                                                    ...
                                                    let KK4KindFiltered2 = KK4Kind.filter(item => item.id_agroforest_kt4 === id);

                                                    if (KK4KindFiltered2.length > 0) {
                                                        for (let i = 0; i < KK4KindFiltered2.length; i++) {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt4`, {
                                                                method: "POST",
                                                                headers: {
                                                                    "authorization": `Bearer ${globalContext.credentials.token}`,
                                                                    "content-type": "application/json"
                                                                },
                                                                body: JSON.stringify(KK4KindFiltered2[i])
                                                            });
                                                            ...
                                                            if (response1.success) {
                                                                ...
                                                                let KK4Kind = await AsyncStorage.getItem("KK4Kind");
                                                                KK4Kind = JSON.parse(KK4Kind) || [];
                                                                let KK4KindFiltered = KK4Kind.filter(item => item.id_agroforest_kt4 !== id);
                                                                await AsyncStorage.setItem("KK4Kind", JSON.stringify(KK4KindFiltered));
                                                        ...
                                                        KK4KindFiltered2 = KK4KindFiltered2[0];
                                                        ...
                                                        try {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt4`, {
                                                                ...
                                                                body: JSON.stringify(KK4KindFiltered2)
                                                            });
