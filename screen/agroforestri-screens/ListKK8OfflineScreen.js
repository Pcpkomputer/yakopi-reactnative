...
export default function ListKK8OfflineScreen(props){

    const focused = useIsFocused();

    const globalContext = useContext(GlobalContext);
    const [credentials,setCredentials] = useState(globalContext.credentials);
    const [KK8, setKK8] = useState(globalContext.KK8);
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("KK8");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputKK8Offline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
          ...
          <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                               ...
                               <View style={{paddingHorizontal:EStyleSheet.value("20rem"),justifyContent:"center",alignItems:"center",paddingVertical:EStyleSheet.value("20rem")}}>
                                ...
                                <View style={{flexDirection:"column",flex:1}}>
                                    ...
                                    <View style={{padding:EStyleSheet.value("10rem")}}>
                                        <Text style={{color:"white",fontWeight:"bold",fontSize:EStyleSheet.value("16rem"),paddingBottom:EStyleSheet.value("10rem")}}>KEGIATAN PENYISIPAN/REPLANTING</Text>
                                    </View>
                                </View>
                                <TouchableOpacity 
                                activeOpacity={0.8}
                            ...
                            <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindKK8Offline",{id_agroforest_kt8:item.id,status:item.status});
                                }}
                                    ...
                                    onPress={async () => {
                                        ...
                                        [
                                            ...
                                            onPress: async () => {
                                                ...
                                                if (list.length > 0) {
                                                list.splice(index, 1);
                                                await AsyncStorage.setItem("KK8", JSON.stringify(list));
                                                setList(list);
                                                setListLoading(false);
                                        ...
                                        onPress={async () => {
                                            ...
                                            try {
                                                ...
                                                if (response.success) {
                                                    ...
                                                    // Update KK8Kind
                                                    let id = response.id_agroforest_kt8;
                                                    let id_agroforest_kt8 = data.id;

                                                    let KK8Kind = await AsyncStorage.getItem("KK8Kind");
                                                    KK8Kind = JSON.parse(KK8Kind) || [];

                                                    let KK8KindFiltered = KK8Kind.filter(item => item.id_agroforest_kt8 === id_agroforest_kt8);

                                                    if (KK8KindFiltered.length > 0) {
                                                        KK8KindFiltered.forEach(item => {
                                                            item.id_agroforest_kt8 = id;
                                                        });
                                                        await AsyncStorage.setItem("KK8Kind", JSON.stringify(KK8Kind));
                                                    } else {
                                                        // KK8Kind tidak ada, tidak perlu mengupload
                                                    ...
                                                    let KK8KindFiltered2 = KK8Kind.filter(item => item.id_agroforest_kt8 === id);

                                                    if (KK8KindFiltered2.length > 0) {
                                                        for (let i = 0; i < KK8KindFiltered2.length; i++) {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt8`, {
                                                                method: "POST",
                                                                headers: {
                                                                    "authorization": `Bearer ${globalContext.credentials.token}`,
                                                                    "content-type": "application/json"
                                                                },
                                                                body: JSON.stringify(KK8KindFiltered2[i])
                                                            });
                                                            ...
                                                            if (response1.success) {
                                                                ...
                                                                let KK8Kind = await AsyncStorage.getItem("KK8Kind");
                                                                KK8Kind = JSON.parse(KK8Kind) || [];
                                                                let KK8KindFiltered = KK8Kind.filter(item => item.id_agroforest_kt8 !== id);
                                                                await AsyncStorage.setItem("KK8Kind", JSON.stringify(KK8KindFiltered));
                                                        ...
                                                        KK8KindFiltered2 = KK8KindFiltered2[0];
                                                        ...
                                                        try {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt8`, {
                                                                ...
                                                                body: JSON.stringify(KK8KindFiltered2)
                                                            });
