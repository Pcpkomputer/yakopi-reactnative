...
export default function ListKK5OfflineScreen(props){
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("KK5");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputKK5Offline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
          ...
          <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                               ...
                               <View style={{paddingHorizontal:EStyleSheet.value("20rem"),justifyContent:"center",alignItems:"center",paddingVertical:EStyleSheet.value("20rem")}}>
                                ...
                                <View style={{flexDirection:"column",flex:1}}>
                                    ...
                                    <View style={{padding:EStyleSheet.value("10rem")}}>
                                        <Text style={{color:"white",fontWeight:"bold",fontSize:EStyleSheet.value("16rem"),paddingBottom:EStyleSheet.value("10rem")}}>KEGIATAN PELANGSIRAN BIBIT & TRANSPORTASI</Text>
                                    </View>
                                </View>
                                <TouchableOpacity 
                                activeOpacity={0.8}
                            ...
                            <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindKK5Offline",{id_agroforest_kt5:item.id,status:item.status});
                                }}
                                    ...
                                    onPress={async () => {
                                        ...
                                        [
                                            ...
                                            onPress: async () => {
                                                ...
                                                if (list.length > 0) {
                                                list.splice(index, 1);
                                                await AsyncStorage.setItem("KK5", JSON.stringify(list));
                                                setList(list);
                                                setListLoading(false);
                                    ...
                                    isConnected ? (
                                        ...
                                        onPress={async () => {
                                            ...
                                            try {
                                                ...
                                                let request = await fetch(`${endpoint}/agroforest-kt5`, {
                                                    method: "POST",
                                                    headers: {
                                                ...
                                                if (response.success) {
                                                    ...
                                                    // Update KK5Kind
                                                    let id = response.id_agroforest_kt5;
                                                    let id_agroforest_kt5 = data.id;

                                                    let KK5Kind = await AsyncStorage.getItem("KK5Kind");
                                                    KK5Kind = JSON.parse(KK5Kind) || [];

                                                    let KK5KindFiltered = KK5Kind.filter(item => item.id_agroforest_kt5 === id_agroforest_kt5);

                                                    if (KK5KindFiltered.length > 0) {
                                                        KK5KindFiltered.forEach(item => {
                                                            item.id_agroforest_kt5 = id;
                                                        });
                                                        await AsyncStorage.setItem("KK5Kind", JSON.stringify(KK5Kind));
                                                    } else {
                                                        // KK5Kind tidak ada, tidak perlu mengupload
                                                    ...
                                                    let KK5KindFiltered2 = KK5Kind.filter(item => item.id_agroforest_kt5 === id);

                                                    if (KK5KindFiltered2.length > 0) {
                                                        for (let i = 0; i < KK5KindFiltered2.length; i++) {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt5`, {
                                                                method: "POST",
                                                                headers: {
                                                                    "authorization": `Bearer ${globalContext.credentials.token}`,
                                                                    "content-type": "application/json"
                                                                },
                                                                body: JSON.stringify(KK5KindFiltered2[i])
                                                            });
                                                            ...
                                                            if (response1.success) {
                                                                ...
                                                                let KK5Kind = await AsyncStorage.getItem("KK5Kind");
                                                                KK5Kind = JSON.parse(KK5Kind) || [];
                                                                let KK5KindFiltered = KK5Kind.filter(item => item.id_agroforest_kt5 !== id);
                                                                await AsyncStorage.setItem("KK5Kind", JSON.stringify(KK5KindFiltered));
                                                        ...
                                                        KK5KindFiltered2 = KK5KindFiltered2[0];
                                                        ...
                                                        try {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt5`, {
                                                                ...
                                                                body: JSON.stringify(KK5KindFiltered2)
                                                            });
