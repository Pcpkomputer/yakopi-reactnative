...
export default function InputDetailKK9OfflineScreen(props){
    ...
    const id_agroforest_kt9 = props.route.params.id_agroforest_kt9;
            ...
            >
               ...
               <TouchableOpacity 
                ...
                onPress={async () => {
                    ...
                    payload.id = props.route.params.id_agroforest_kt9;
                    let payloadArray = [payload];

                    try {
                    const storedKK9Kind = await AsyncStorage.getItem("KK9Kind");
                    const parsedStoredKK9Kind = JSON.parse(storedKK9Kind) || [];

                    if (parsedStoredKK9Kind.length > 0) {
                        const updatedKK9Kind = [...parsedStoredKK9Kind, ...payloadArray];
                        await AsyncStorage.setItem("KK9Kind", JSON.stringify(updatedKK9Kind));
                        setKK9Kind(updatedKK9Kind);
                    } else {
                        await AsyncStorage.setItem("KK9Kind", JSON.stringify(payloadArray));
                        setKK9Kind(payloadArray);
                    }

                    setSmokeScreenOpened(false);
                    alert('Berhasil menyimpan data ke local');
                    props.navigation.navigate("KindKK9Offline", { id_agroforest_kt9: props.route.params.id_agroforest_kt9 });
                    } catch (error) {
