...
export default function InputDetailKK8OfflineScreen(props){
    ...
    const [KK8Kind, setKK8Kind] = useState(globalContext.KK8Kind);
    const id_agroforest_kt8 = props.route.params.id_agroforest_kt8;
    ...
    const [schema, setSchema] = useState([
        {
            form:"id_agroforest_kt8",
            value:props.route.params.id_agroforest_kt8,
        },
        {
            type:"textinput",
            label:"Site Code",
            ...
            >
               ...
               <TouchableOpacity 
                ...
                onPress={async () => {
                    ...
                    payload.id = props.route.params.id_agroforest_kt8;
                    let payloadArray = [payload];

                    try {
                    const storedKK8Kind = await AsyncStorage.getItem("KK8Kind");
                    const parsedStoredKK8Kind = JSON.parse(storedKK8Kind) || [];

                    if (parsedStoredKK8Kind.length > 0) {
                        const updatedKK8Kind = [...parsedStoredKK8Kind, ...payloadArray];
                        await AsyncStorage.setItem("KK8Kind", JSON.stringify(updatedKK8Kind));
                        setKK8Kind(updatedKK8Kind);
                    } else {
                        await AsyncStorage.setItem("KK8Kind", JSON.stringify(payloadArray));
                        setKK8Kind(payloadArray);
                    }

                    setSmokeScreenOpened(false);
                    alert('Berhasil menyimpan data ke local');
                    props.navigation.navigate("KindKK8Offline", { id_agroforest_kt8: props.route.params.id_agroforest_kt8 });
                    } catch (error) {
