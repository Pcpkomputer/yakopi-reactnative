...
export default function InputDetailKK6OfflineScreen(props){
    ...
    const [KK6Kind, setKK6Kind] = useState(globalContext.KK6Kind);
    const id_agroforest_kt6 = props.route.params.id_agroforest_kt6;
    ...
    const [schema, setSchema] = useState([
        {
            form:"id_agroforest_kt6",
            value:props.route.params.id_agroforest_kt6,
        },
        {
            type:"textinput",
            label:"Site Code",
            ...
            >
               ...
               <TouchableOpacity 
                ...
                onPress={async () => {
                    ...
                    payload.id = props.route.params.id_agroforest_kt6;
                    let payloadArray = [payload];

                    try {
                    const storedKK6Kind = await AsyncStorage.getItem("KK6Kind");
                    const parsedStoredKK6Kind = JSON.parse(storedKK6Kind) || [];

                    if (parsedStoredKK6Kind.length > 0) {
                        const updatedKK6Kind = [...parsedStoredKK6Kind, ...payloadArray];
                        await AsyncStorage.setItem("KK6Kind", JSON.stringify(updatedKK6Kind));
                        setKK6Kind(updatedKK6Kind);
                    } else {
                        await AsyncStorage.setItem("KK6Kind", JSON.stringify(payloadArray));
                        setKK6Kind(payloadArray);
                    }

                    setSmokeScreenOpened(false);
                    alert('Berhasil menyimpan data ke local');
                    props.navigation.navigate("KindKK6Offline", { id_agroforest_kt6: props.route.params.id_agroforest_kt6 });
                    } catch (error) {
