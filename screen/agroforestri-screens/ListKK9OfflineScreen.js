...
export default function ListKK9OfflineScreen(props){
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("KK9");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputKK9Offline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
          ...
          <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                            ...
                            <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindKK9Offline",{id_agroforest_kt9:item.id,status:item.status});
                                }}
                                    ...
                                    onPress={async () => {
                                        ...
                                        [
                                            ...
                                            onPress: async () => {
                                                ...
                                                if (list.length > 0) {
                                                list.splice(index, 1);
                                                await AsyncStorage.setItem("KK9", JSON.stringify(list));
                                                setList(list);
                                                setListLoading(false);
                                        ...
                                        onPress={async () => {
                                            ...
                                            try {
                                                ...
                                                if (response.success) {
                                                    ...
                                                    // Update KK9Kind
                                                    let id = response.id_agroforest_kt9;
                                                    let id_agroforest_kt9 = data.id;

                                                    let KK9Kind = await AsyncStorage.getItem("KK9Kind");
                                                    KK9Kind = JSON.parse(KK9Kind) || [];

                                                    let KK9KindFiltered = KK9Kind.filter(item => item.id_agroforest_kt9 === id_agroforest_kt9);

                                                    if (KK9KindFiltered.length > 0) {
                                                        KK9KindFiltered.forEach(item => {
                                                            item.id_agroforest_kt9 = id;
                                                        });
                                                        await AsyncStorage.setItem("KK9Kind", JSON.stringify(KK9Kind));
                                                    } else {
                                                        // KK9Kind tidak ada, tidak perlu mengupload
                                                    ...
                                                    let KK9KindFiltered2 = KK9Kind.filter(item => item.id_agroforest_kt9 === id);

                                                    if (KK9KindFiltered2.length > 0) {
                                                        for (let i = 0; i < KK9KindFiltered2.length; i++) {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt9`, {
                                                                method: "POST",
                                                                headers: {
                                                                    "authorization": `Bearer ${globalContext.credentials.token}`,
                                                                    "content-type": "application/json"
                                                                },
                                                                body: JSON.stringify(KK9KindFiltered2[i])
                                                            });
                                                            ...
                                                            if (response1.success) {
                                                                ...
                                                                let KK9Kind = await AsyncStorage.getItem("KK9Kind");
                                                                KK9Kind = JSON.parse(KK9Kind) || [];
                                                                let KK9KindFiltered = KK9Kind.filter(item => item.id_agroforest_kt9 !== id);
                                                                await AsyncStorage.setItem("KK9Kind", JSON.stringify(KK9KindFiltered));
                                                        ...
                                                        KK9KindFiltered2 = KK9KindFiltered2[0];
                                                        ...
                                                        try {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt9`, {
                                                                ...
                                                                body: JSON.stringify(KK9KindFiltered2)
                                                            });
