...
export default function ListKK2OfflineScreen(props){
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("KK2");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputKK2Offline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
          ...
          <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                            ...
                            <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindKK2Offline",{id_agroforest_kt2:item.id,status:item.status});
                                }}
                                    ...
                                    onPress={async () => {
                                        ...
                                        [
                                            ...
                                            onPress: async () => {
                                                ...
                                                if (list.length > 0) {
                                                list.splice(index, 1);
                                                await AsyncStorage.setItem("KK2", JSON.stringify(list));
                                                setList(list);
                                                setListLoading(false);
                                    ...
                                    isConnected ? (
                                        ...
                                        onPress={async () => {
                                            ...
                                            try {
                                                ...
                                                let request = await fetch(`${endpoint}/agroforest-kt2`, {
                                                    method: "POST",
                                                    headers: {
                                                ...
                                                if (response.success) {
                                                    ...
                                                    // Update KK2Kind
                                                    let id = response.id_agroforest_kt2;
                                                    let id_agroforest_kt2 = data.id;

                                                    let KK2Kind = await AsyncStorage.getItem("KK2Kind");
                                                    KK2Kind = JSON.parse(KK2Kind) || [];

                                                    let KK2KindFiltered = KK2Kind.filter(item => item.id_agroforest_kt2 === id_agroforest_kt2);

                                                    if (KK2KindFiltered.length > 0) {
                                                        KK2KindFiltered.forEach(item => {
                                                            item.id_agroforest_kt2 = id;
                                                        });
                                                        await AsyncStorage.setItem("KK2Kind", JSON.stringify(KK2Kind));
                                                    } else {
                                                        // KK2Kind tidak ada, tidak perlu mengupload
                                                    ...
                                                    let KK2KindFiltered2 = KK2Kind.filter(item => item.id_agroforest_kt2 === id);

                                                    if (KK2KindFiltered2.length > 0) {
                                                        for (let i = 0; i < KK2KindFiltered2.length; i++) {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt2`, {
                                                                method: "POST",
                                                                headers: {
                                                                    "authorization": `Bearer ${globalContext.credentials.token}`,
                                                                    "content-type": "application/json"
                                                                },
                                                                body: JSON.stringify(KK2KindFiltered2[i])
                                                            });
                                                            ...
                                                            if (response1.success) {
                                                                ...
                                                                let KK2Kind = await AsyncStorage.getItem("KK2Kind");
                                                                KK2Kind = JSON.parse(KK2Kind) || [];
                                                                let KK2KindFiltered = KK2Kind.filter(item => item.id_agroforest_kt2 !== id);
                                                                await AsyncStorage.setItem("KK2Kind", JSON.stringify(KK2KindFiltered));
                                                        ...
                                                        KK2KindFiltered2 = KK2KindFiltered2[0];
                                                        ...
                                                        try {
                                                            let request1 = await fetch(`${endpoint}/add-kind-agroforest-kt2`, {
                                                                ...
                                                                body: JSON.stringify(KK2KindFiltered2)
                                                            });
