...
export default function ListCutiScreen(props){
    ...
    let fetchList = async () =>{
        setListLoading(true);
        let request = await fetch(`${endpoint}/get-cuti`,{
            method:"POST",
            headers:{
    ...
    return (
        <View style={{flex:1}}>
            ...
            activeOpacity={0.6}
            onPress={()=>{
                props.navigation.navigate("InputCuti");
            }}
                ...
                <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                            ...
                            activeOpacity={0.7}
                            onPress={()=>{
                                props.navigation.navigate("DetailCuti",{item:item});
                            }}
                            ...
                            style={{flexDirection:"row",marginHorizontal:EStyleSheet.value("20rem"),borderRadius:EStyleSheet.value("5rem")}}>
                                <View style={{paddingHorizontal:EStyleSheet.value("20rem"),justifyContent:"center",alignItems:"center",paddingVertical:EStyleSheet.value("20rem")}}>
                                    <Text style={{color:"white",fontSize:EStyleSheet.value("20rem"),fontWeight:"bold"}}>{item.id_cuti}</Text>
                                </View>
                                <View style={{flexDirection:"column",flex:1}}>
                                    <View style={{padding:EStyleSheet.value("10rem")}}>
                                        <Text style={{color:"white",fontWeight:"bold",fontSize:EStyleSheet.value("16rem")}}>Cuti Tanggal :</Text> 
                                        <Text style={{color:"white",fontWeight:"bold",fontSize:EStyleSheet.value("16rem")}}>{item.tanggal_mulai_cuti} - {item.tanggal_selesai_cuti}</Text>
                                    </View>
                                    <ScrollView showsHorizontalScrollIndicator={false} horizontal={true} style={{paddingHorizontal:EStyleSheet.value("10rem"),flexDirection:"row"}}>
                                        ...
                                        <Text style={{fontSize:EStyleSheet.value("11rem"),color:"white"}}>{item.status_cuti}</Text>
                                        </View>
                                        ...
                                        <Text style={{fontSize:EStyleSheet.value("11rem"),color:"white"}}>{item.nama_kategori_cuti}</Text>
                                    </View>
                                    </ScrollView>
                            ...
                            <View style={{marginHorizontal:EStyleSheet.value("20rem"),flexDirection:"row",justifyContent:"space-around",padding:EStyleSheet.value("10rem"),backgroundColor:"#DDDDDD"}}>
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_cuti;
                                                    let request = await fetch(`${endpoint}/delete-cuti`,{
                                                        method:"DELETE",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                ...
                                {
                                    (item.status_cuti==='Pending' && item.created_by===globalContext.credentials.data.id_pengguna) &&
                                    <TouchableOpacity 
                                    onPress={async ()=>{
                                        Alert.alert(
                                            "Dialog Konfirmasi",
                                            "Anda yakin ingin mengajukan data ini?",
                                            [
                                              {
                                                text: "Tidak",
                                                style: "cancel"
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_cuti;
                                                    let request = await fetch(`${endpoint}/pengajuan-cuti`,{
                                                        method:"PUT",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
