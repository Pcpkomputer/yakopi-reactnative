...
export default function ListCCBOfflineScreen(props){

    const focused = useIsFocused();

    const globalContext = useContext(GlobalContext);
    const [credentials,setCredentials] = useState(globalContext.credentials);
    const [ccb, setCCB] = useState(globalContext.ccb);
    ...
    let fetchList = async () => {
        ...
        let list = await AsyncStorage.getItem("ccb");
    ...
    return (
        <View style={{ flex: 1 }}>
          <TouchableOpacity
            activeOpacity={0.6}
            onPress={() => {
              props.navigation.navigate("InputCCBOffline");
            }}
            style={{
              position: "absolute",
              zIndex: 9999,
                                    ...
                                    Alert.alert(
                                        ...
                                        [
                                            ...
                                            { text: "Iya", onPress: async () => {
                                                ...
                                                list.splice(index,1);
                                                await AsyncStorage.setItem("ccb",JSON.stringify(list));
                                ...
                                </TouchableOpacity>
                                {
                                    isConnected ? (
                                        ...
                                        onPress={async ()=>{
                                            ...
                                            let request = await fetch(`${endpoint}/research/ccb/add`,{
                                                method:"POST",
                                                headers:{
