...
export default function ListPlanktonScreen(props){
    ...
    let fetchList = async () =>{
        setListLoading(true);
        let request = await fetch(`${endpoint}/research/plankton`,{
            method:"GET",
            headers:{
    ...
    return (
        <View style={{flex:1}}>
            ...
            activeOpacity={0.6}
            onPress={()=>{
                props.navigation.navigate("InputPlankton");
            }}
                ...
                <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                            ...
                            activeOpacity={0.7}
                            onPress={()=>{
                                props.navigation.navigate("DetailPlankton",{item:item});
                            }}
                            key={item.id_biodiversity_planton}>
                            <LinearGradient
                            ...
                            style={{flexDirection:"row",marginHorizontal:EStyleSheet.value("20rem"),borderRadius:EStyleSheet.value("5rem")}}>
                                <View style={{paddingHorizontal:EStyleSheet.value("20rem"),justifyContent:"center",alignItems:"center",paddingVertical:EStyleSheet.value("20rem")}}>
                                    <Text style={{color:"white",fontSize:EStyleSheet.value("20rem"),fontWeight:"bold"}}>{item.id_biodiversity_planton}</Text>
                                </View>
                                ...
                                onPress={()=>{
                                    ...
                                    var url = scheme + `${item.lat_biodiversity},${item.long_biodiversity}`;
                                    Linking.openURL(url);
                                }}
                            ...
                            <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindZooplankton",{id_plankton:item.id_biodiversity_planton,status:item.status});
                                }}
                                style={{backgroundColor:"#9ed649",borderRadius:EStyleSheet.value("5rem"),paddingHorizontal:EStyleSheet.value("10rem"),paddingVertical:EStyleSheet.value("5rem")}}>
                                    <Text style={{ color: "white" }}>Zooplankton</Text>
                                </TouchableOpacity>
                                <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindFitoplankton",{id_plankton:item.id_biodiversity_planton,status:item.status});
                                }}
                                style={{backgroundColor:"blue",borderRadius:EStyleSheet.value("5rem"),paddingHorizontal:EStyleSheet.value("10rem"),paddingVertical:EStyleSheet.value("5rem")}}>
                                     <Text style={{ color: "white" }}>Fitoplankton</Text>
                                </TouchableOpacity>
                                <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindPlanktonFiskim",{id_plankton:item.id_biodiversity_planton,status:item.status});
                                }}
                                style={{backgroundColor:"red",borderRadius:EStyleSheet.value("5rem"),paddingHorizontal:EStyleSheet.value("10rem"),paddingVertical:EStyleSheet.value("5rem")}}>
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_biodiversity_planton;
                                                    let request = await fetch(`${endpoint}/research/plankton/delete`,{
                                                        method:"DELETE",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_biodiversity_planton;
                                                    let request = await fetch(`${endpoint}/research/approve-plankton`,{
                                                        method:"POST",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
                            ...
                            <View style={{marginHorizontal:EStyleSheet.value("20rem"),justifyContent:"space-around",padding:EStyleSheet.value("10rem"),backgroundColor:"#DDDDDD"}}>
                                <TouchableOpacity
                                onPress={()=>{
                                    props.navigation.navigate("UpdatePlankton",{item:item});
                                }}
