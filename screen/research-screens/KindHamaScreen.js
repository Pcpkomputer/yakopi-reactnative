...
export default function KindHamaScreen(props){
    ...
    let fetchList = async () =>{
        setListLoading(true);
        let id = props.route.params.id_hama;
        let request = await fetch(`${endpoint}/research/detailHama`,{
            method:"POST",
            headers:{
    ...
    return (
      <View style={{flex:1, backgroundColor:'#fff'}}>
        ...
        <TouchableOpacity 
            activeOpacity={0.6}
            onPress={()=>{
                props.navigation.navigate("InputDetailHama",{ id_hama:props.route.params.id_hama });
            }}
        ...
        list.map((item, index) => (
          <DataTable.Row key={index}>
            <DataTable.Cell>
            {
                  ...
                  Alert.alert(
                      ...
                      [
                        ...
                        { text: "Iya", onPress: async () => {
                              ...
                              let id = item.id_detail_hama;
                              let request = await fetch(`${endpoint}/research/hama/deleteDetail`,{
                                  method:"DELETE",
                                  headers:{
                                      "authorization":`Bearer ${globalContext.credentials.token}`,
                                      "content-type":"application/json"
              ...
              <TouchableOpacity 
                onPress={()=>{
                    props.navigation.navigate("AssetHama",{type:"image",id_detail_hama:item.id_detail_hama,status:props.route.params.status});
                }}
          ...
          }
          {
            ...
            <TouchableOpacity 
              ...
              onPress={()=>{
                  ...
                  var url = scheme + `${item.latitude_hama},${item.longtitude_hama}`;
                  Linking.openURL(url);
              }}
            ...
            <DataTable.Cell>{item.jenis_hama}</DataTable.Cell>
