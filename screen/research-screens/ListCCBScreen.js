...
export default function ListCCBScreen(props){
    ...
    let fetchList = async () =>{
        setListLoading(true);
        let request = await fetch(`${endpoint}/research/ccb`,{
            method:"GET",
            headers:{
    ...
    return (
        <View style={{flex:1}}>
            ...
            activeOpacity={0.6}
            onPress={()=>{
                props.navigation.navigate("InputCCB");
            }}
                ...
                <ScrollView contentContainerStyle={{paddingTop:EStyleSheet.value("20rem")}}>
                 {
                     list.map((item,index)=>{
                         return (
                            ...
                            activeOpacity={0.7}
                            onPress={()=>{
                                props.navigation.navigate("DetailCCB",{item:item});
                            }}
                            key={item.id_ccb}>
                            <LinearGradient
                            ...
                            style={{flexDirection:"row",marginHorizontal:EStyleSheet.value("20rem"),borderRadius:EStyleSheet.value("5rem")}}>
                                <View style={{paddingHorizontal:EStyleSheet.value("20rem"),justifyContent:"center",alignItems:"center",paddingVertical:EStyleSheet.value("20rem")}}>
                                    <Text style={{color:"white",fontSize:EStyleSheet.value("20rem"),fontWeight:"bold"}}>{item.id_ccb}</Text>
                                </View>
                                <View style={{flexDirection:"column",flex:1}}>
                                    ...
                                    <View style={{padding:EStyleSheet.value("10rem")}}>
                                        <Text style={{color:"white",fontWeight:"bold",fontSize:EStyleSheet.value("16rem"),paddingBottom:EStyleSheet.value("10rem")}}>CARBON</Text>
                                    </View>
                                </View>
                                <TouchableOpacity 
                                activeOpacity={0.8}
                            ...
                            <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindCarbonCCB",{id_ccb:item.id_ccb,status:item.status});
                                }}
                                style={{backgroundColor:"#9ed649",borderRadius:EStyleSheet.value("5rem"),paddingHorizontal:EStyleSheet.value("10rem"),paddingVertical:EStyleSheet.value("5rem")}}>
                                    <Text style={{ color: "white" }}>CARBON</Text>
                                </TouchableOpacity>
                                <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindFloraCCB",{id_ccb:item.id_ccb,status:item.status});
                                }}
                                style={{backgroundColor:"blue",borderRadius:EStyleSheet.value("5rem"),paddingHorizontal:EStyleSheet.value("10rem"),paddingVertical:EStyleSheet.value("5rem")}}>
                                     <Text style={{ color: "white" }}>FLORA</Text>
                                </TouchableOpacity>
                                <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindFaunaCCB",{id_ccb:item.id_ccb,status:item.status});
                                }}
                                ...
                                <TouchableOpacity 
                                 onPress={()=>{
                                    props.navigation.navigate("KindFiskimCCB",{id_ccb:item.id_ccb,status:item.status});
                                }}
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_ccb;
                                                    let request = await fetch(`${endpoint}/research/ccb/delete`,{
                                                        method:"DELETE",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_ccb;
                                                    let request = await fetch(`${endpoint}/research/approve-ccb`,{
                                                        method:"POST",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
                            ...
                            <View style={{marginHorizontal:EStyleSheet.value("20rem"),justifyContent:"space-around",padding:EStyleSheet.value("10rem"),backgroundColor:"#DDDDDD"}}>
                                <TouchableOpacity
                                onPress={()=>{
                                    props.navigation.navigate("UpdateCCB",{item:item});
                                }}
