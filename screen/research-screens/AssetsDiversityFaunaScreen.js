...
function AssetsImage(props){
    ...
    const fetchImage = async()=>{
        ...
        let id_detail_diversity_fauna = props.route.params.id_detail_diversity_fauna;
        let request = await fetch(`${endpoint}/research/photoDataDiversityFauna`,{
            method:"POST",
            headers:{
            ...
            body:JSON.stringify({
                id_detail_diversity_fauna:id_detail_diversity_fauna
            })
        });
    ...
    useEffect(()=>{
        props.navigation.setOptions({
            headerTitle: `Photo Diversity Fauna - ${props.route.params.id_detail_diversity_fauna}`,
        });
    },[])
                    ...
                    <View style={{...shadow,backgroundColor:"white",width:Dimensions.get("screen").width-EStyleSheet.value("60rem"),borderRadius:EStyleSheet.value("5rem"),padding:EStyleSheet.value("20rem")}}>
                        ...
                        onPress={async ()=>{
                            ...
                            if(!gambar.cancelled){
                                ...
                                form.append("file_diversity_fauna",photo);
                                let request = await fetch(`https://sispro-yakopi.org/endpoint/dokumentasiPhotoDiversityFauna`,{
                                    method:"POST",
                                    body:form
                                });
                                let response = await request.json();
                                ...
                                let req2 = await fetch(`${endpoint}/research/diversityFauna/addPhoto`,{
                                    method:"POST",
                                    headers:{
                                        "authorization":`Bearer ${globalContext.credentials.token}`,
                                        "content-type":"application/json"
                                    ...
                                    body:JSON.stringify({
                                        id_detail_diversity_fauna:id_detail_diversity_fauna,
                                        keterangan_diversity_fauna:keterangan,
                                        link_diversity_fauna:"",
                                        file_diversity_fauna:url
                                    })
                                });
                                console.log(req2);
                        ...
                        return (
                            <View style={{marginHorizontal:EStyleSheet.value("20rem"),overflow:"hidden",borderRadius:EStyleSheet.value("3rem"),marginBottom:EStyleSheet.value("15rem")}}>
                                <View style={{backgroundColor:"#e8e8e8",height:EStyleSheet.value("200rem")}}>
                                    {
                                        (item.link_diversity_fauna===null || item.link_diversity_fauna==="") ?
                                        <Image style={{width:"100%",height:"100%"}} source={{uri:`${endpoint.replace(/api(.?)/g,"")}${item.file_diversity_fauna}`}}></Image>
                                        :
                                        <Image style={{width:"100%",height:"100%"}} source={{uri:item.link_diversity_fauna}}></Image>
                                    }
                                </View>
                                <View style={{position:"absolute",zIndex:10,bottom:EStyleSheet.value("20rem"),paddingHorizontal:EStyleSheet.value("20rem")}}>
                                    <Text style={{color:"white",fontSize:EStyleSheet.value("20rem")}}>{item.keterangan_diversity_fauna}</Text>
                                </View>
                                <LinearGradient
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_diversity_fauna_photo;
                                                    let request = await fetch(`${endpoint}/research/diversityFauna/deletePhoto`,{
                                                        method:"DELETE",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
...
export default function AssetsDiversityFaunaScreen(props){
