...
function AssetsImage(props){
    ...
    const fetchImage = async()=>{
        ...
        let id = props.route.params.id_detail_growth_research;
        let request = await fetch(`${endpoint}/research/photoDataGrowthResearch`,{
            method:"POST",
            headers:{
    ...
    useEffect(()=>{
        props.navigation.setOptions({
            headerTitle: `Photo Growth Research - ${props.route.params.id_detail_growth_research}`,
        });
    },[])
                    ...
                    <View style={{...shadow,backgroundColor:"white",width:Dimensions.get("screen").width-EStyleSheet.value("60rem"),borderRadius:EStyleSheet.value("5rem"),padding:EStyleSheet.value("20rem")}}>
                        ...
                        onPress={async ()=>{
                            ...
                            if(!gambar.canceled){
                                ...
                                form.append("file_growth_research_photo",photo);
                                let request = await fetch(`https://sispro-yakopi.org/endpoint/dokumentasiPhotoGrowthResearch`,{
                                    method:"POST",
                                    body:form
                                });
                                let response = await request.json();
            
                                let url = `/assets/img/photoGrowthResearch/${response.result.orig_name}`;

                                let id_detail_growth_research = props.route.params.id_detail_growth_research;
            
                                let req2 = await fetch(`${endpoint}/research/growthResearch/addPhoto`,{
                                    method:"POST",
                                    headers:{
                                        "authorization":`Bearer ${globalContext.credentials.token}`,
                                        "content-type":"application/json"
                                    ...
                                    body:JSON.stringify({
                                        id_detail_growth_research:id_detail_growth_research,
                                        keterangan_growth_research:keterangan,
                                        link_growth_research_photo:"",
                                        file_growth_research_photo:url
                                    })
                                });
                                console.log(req2);
                        ...
                        return (
                            <View style={{marginHorizontal:EStyleSheet.value("20rem"),overflow:"hidden",borderRadius:EStyleSheet.value("3rem"),marginBottom:EStyleSheet.value("15rem")}}>
                                <View style={{backgroundColor:"#e8e8e8",height:EStyleSheet.value("200rem")}}>
                                    {
                                        (item.link_growth_research_photo===null || item.link_growth_research_photo==="") ?
                                        <Image style={{width:"100%",height:"100%"}} source={{uri:`${endpoint.replace(/api(.?)/g,"")}${item.file_growth_research_photo}`}}></Image>
                                        :
                                        <Image style={{width:"100%",height:"100%"}} source={{uri:item.link_growth_research_photo}}></Image>
                                    }
                                </View>
                                <View style={{position:"absolute",zIndex:10,bottom:EStyleSheet.value("20rem"),paddingHorizontal:EStyleSheet.value("20rem")}}>
                                    <Text style={{color:"white",fontSize:EStyleSheet.value("20rem")}}>{item.keterangan_growth_research}</Text>
                                </View>
                                <LinearGradient
                                        ...
                                        Alert.alert(
                                            ...
                                            [
                                              ...
                                              { text: "Iya", onPress: async () => {
                                                    ...
                                                    let id = item.id_growth_research_photo;
                                                    let request = await fetch(`${endpoint}/research/growthResearch/deletePhoto`,{
                                                        method:"DELETE",
                                                        headers:{
                                                            "authorization":`Bearer ${globalContext.credentials.token}`,
                                                            "content-type":"application/json"
...
export default function AssetsGrowthResearchScreen(props){
